...
class main_module
{
	...
	protected function campaigns($id, $mode)
	{
		...
		$guarded = array('start', 'pause', 'resume', 'cancel', 'relance', 'retry_errors', 'send_now', 'resend_all', 'resend_unconfirmed', 'resend_one', 'send_test');
	...
	protected function campaign_start($campaign_id)
	{
		...
		else
		{
			...
			$sql = 'SELECT DISTINCT u.user_id, u.username, u.user_email, u.user_type,
			...
			$excluded = $ex_deactivated + $ex_inactive + $ex_massemail;
	...
	protected function campaign_details($campaign_id)
	{
		...
		$nb_pending = $nb_sent = $nb_error = $nb_confirmed = 0;
		...
		(
			...
			'NB_TOTAL'		=> $nb_pending + $nb_sent + $nb_error,
	...
	protected function campaign_delete($campaign_id)
	{
		...
		// Un brouillon, ou une campagne arrêtée dont rien n'est jamais parti,
		// peut être supprimé sans perdre d'historique.
		if ($status !== 'draft' && !($status === 'cancelled' && $nb_done === 0))
		{
	...
	protected function diagnostics()
	{
		...
		$line = function($label, $value, $state) use (&$rows) {
		...
		$expected = array(
			...
			$this->queue_table => array('confirm_token', 'confirmed_time'),
		);
	...
	protected function history($id, $mode)
	{
		...
		$sql = 'SELECT q.queue_id, q.username, q.email, q.status, q.sent_time, q.error_message,
				q.confirmed_time, c.require_confirm,
